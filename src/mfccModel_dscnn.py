...
def getData():
    trainF = np.load('input/mfccTrainFeatures.npy').reshape(-1,20,44,1)
    testF = np.load('input/mfccTestFeatures.npy').reshape(-1,20,44,1)
...
def create_model():
    model = Sequential()
    model.add(SeparableConv2D(32, kernel_size=(2,2), activation='relu', input_shape=(20, 44, 1)))
