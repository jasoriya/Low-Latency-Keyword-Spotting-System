...
sys.path.append('../src')
sys.path.append('../src/input/picts/train')
...
def get_jpeg_data_files_paths():
    ...
    data_root_folder = os.path.abspath("../src/input/picts/")
    train_jpeg_dir = os.path.join(data_root_folder, 'train/')
    test_jpeg_dir = os.path.join(data_root_folder, 'test/')
    train_csv_file = os.path.join(data_root_folder, 'labelsTrain.csv')
    test_csv_file = os.path.join(data_root_folder, 'labelsTest.csv')
...
def get_train_generator(batch_size, filenames, labels_df, train_jpeg_dir):
        ...
        datagen = ImageDataGenerator(
            rescale=1./255,
            shear_range=0.2)  # randomly flip images horizontally
...
labels_df = labels_df.drop("fileName", axis = 1)
trueTestLabels = pd.read_csv(test_csv_file)
test_labels  = pd.DataFrame(trueTestLabels.fileName)
